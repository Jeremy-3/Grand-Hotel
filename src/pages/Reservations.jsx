...
import { FaCalendarCheck, FaPlus, FaCreditCard, FaTimesCircle, FaConciergeBell, FaBed } from 'react-icons/fa';

const Reservations = () => {
  const { user, isAuthenticated, isStaffOrManager } = useAuth();
  ...
  const fetchReservations = async () => {
    ...
    try {
      ...
      if (isStaffOrManager) {
        ...
        res = await reservationsApi.getGuestReservations(user?.id || 1).catch(() => {
          // fallback to all if guest endpoint not ready
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-10">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-gold-500/10 border border-gold-500/30 text-gold-400 text-xs font-semibold uppercase tracking-widest mb-2">
              <FaConciergeBell /> {isStaffOrManager ? 'Management Portal' : 'Guest Portal'}
            </div>
            <h1 className="font-serif text-3xl sm:text-4xl font-bold text-white">
              {isStaffOrManager ? 'All Hotel Bookings' : 'My Reservations'}
            </h1>
            <p className="text-sm text-gray-400 mt-1">
              View and manage confirmed bookings, deposit receipts, and check-in schedules.
            </p>
          </div>
          ...
          <>
            ...
            {loading ? (
              ...
              <div className="text-center py-20 bg-slate-900/40 rounded-3xl border border-slate-800 max-w-lg mx-auto space-y-4">
                ...
                <div>
                  <h3 className="font-serif text-xl font-bold text-white">No Reservations Found</h3>
                  <p className="text-sm text-gray-400 mt-1">
                    {statusFilter !== 'all'
              ...
              <div className="space-y-4">
                {filteredReservations.map((resItem) => {
                  const nights = calculateNights(resItem.check_in_date, resItem.check_out_date);
                  const totalPrice = (resItem.room_price_per_night || 150) * nights;
                  const isPending = resItem.status?.toLowerCase() === 'pending';
                  const isCancellable = ['pending', 'confirmed'].includes(resItem.status?.toLowerCase());
                  ...
                  return (
                    ...
                    >
                        ...
                        <div className="space-y-1.5 min-w-0">
                          <div className="flex items-center gap-3">
                            <h3 className="font-serif text-lg font-bold text-white">
                              Room {resItem.room?.room_number || resItem.room_id}
                            </h3>
                            <StatusBadge status={resItem.status} />
                          </div>

                          <div className="text-xs text-gray-300">
                            <span className="font-medium text-white">{formatDate(resItem.check_in_date)}</span>
                            <span className="text-gray-500 mx-1.5">→</span>
                            <span className="font-medium text-white">{formatDate(resItem.check_out_date)}</span>
                            <span className="text-gold-400 font-semibold ml-2">({nights} {nights === 1 ? 'Night' : 'Nights'})</span>
                          </div>

                          {isStaffOrManager && resItem.guest?.user && (
                            <div className="text-xs text-gray-400">
                              Guest: <strong className="text-gray-200">{resItem.guest.user.name}</strong> ({resItem.guest.user.email})
                            </div>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 border-y sm:border-y-0 sm:border-l sm:border-r border-slate-800 py-3 sm:py-0 sm:px-6">
                        <div>
                          <div className="text-[11px] text-gray-400 uppercase tracking-wider">Nightly Rate</div>
                        ...
                        <div>
                          <div className="text-[11px] text-gray-400 uppercase tracking-wider">Total Amount</div>
                          <div className="text-sm font-bold text-white">{formatCurrency(totalPrice)}</div>
                        </div>

                        <div>
                          <div className="text-[11px] text-gray-400 uppercase tracking-wider">Required Deposit</div>
                          <div className="text-sm font-bold text-gold-400">
                            {formatCurrency(resItem.deposit_amount)} ({resItem.deposit_percentage || 20}%)
                          </div>
                        </div>
                      </div>
